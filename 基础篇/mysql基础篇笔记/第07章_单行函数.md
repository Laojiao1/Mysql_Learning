...
```sql
SELECT employee_id, DATEDIFF(CURDATE(), hire_data) / 365 "Worked_years",
                    DATEDIFF(CURDATE(), hire_data)  "Worked_days"
FROM employees;
...
#WHERE hire_date >= STR_TO_DATE('1997-01-01', '%Y-%m-%d') # 字符串->日期
WHERE DATE_FORMAT(hire_day, '%Y') >= 1997 # 格式化
...
```sql
SETECT last_name, hire_date
FROM employees
WHERE DATADIFF(CURDATE(), hire_date) > 10000;
```
...
SELECT last_name "Last_name", job_id "Job_id", CASE job_id WHEN 'AD_PRES' THEN 'A'
														   WHEN 'ST_MAN' THEN 'A'											                       WHEN 'IT_PROG' THEN 'C'
														   WHEN 'SA_REP' THEN 'D'
