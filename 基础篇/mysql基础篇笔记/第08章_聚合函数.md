...
```sql
SELECT MAX(salary) min_sal, MIN(salary) max_sal, AVG(salary) avg_sal, SUM(salary) sum_salary
FROM employees;
```
...
```sql
SELECT job_id, MAX(salary) min_sal, MIN(salary) max_sal, AVG(salary) avg_sal, SUM(salary) sum_salary
FROM employees
GROUP BY job_id;
```
...
```sql
SELECT MAX(salary) - MIN(salary) "DATEDIFF"
FROM employees;
```
...
```sql
SELECT d.department_id, d.location_id, COUNT(employee_id), AVG(salary)
...
```sql
SELECT d.department_id, e.job_id, MIN(salary)
FROM departments d LEFT JOIN employees e
ON d.`department_id` = e.`department_id`
